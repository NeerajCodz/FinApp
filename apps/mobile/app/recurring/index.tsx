...
export default function RecurringScreen() {
  ...
  const first = enabled
    .map((rule) => {
      ...
      for (let i = 0; date <= now && i < 1024; i++)
        date = nextOccurrence(date, rule.frequency, rule.interval);
      return date;
    })
  ...
  async function createRule() {
    ...
    try {
      amountMinor = parseMinor(amount, selected.currency);
    } catch {
  ...
  async function toggle(rule: Rule) {
    ...
    try {
      await commitLocalWrite(
        ...
        {
          recordId: rule.id,
          dependencies: rule.id.startsWith('local-') ? [`recurringRule:${rule.id}`] : [],
        },
      );
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Could not update reminder.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      {rules.error || accounts.error ? (
        ...
        (rules.loading || accounts.loading) && (
      ...
      {ordered && ordered.length > 0 && (
        <View style={{ gap: 6 }}>
          <Typography variant="label">
            {enabled.length} active · {ordered.length - enabled.length} paused
          </Typography>
          {first && (
            <Typography variant="heading">
              Next due {new Date(first).toLocaleDateString()}
            </Typography>
          )}
          {ordered.map((rule) => {
              ...
              >
                ...
                <View style={{ flex: 1, gap: 4 }}>
                  ...
                  {template?.amountMinor !== undefined && template.currency && (
                    <Money
                      amountMinor={BigInt(template.amountMinor)}
                ...
                <TouchableOpacity
                  ...
                  accessibilityLabel={`${rule.enabled ? 'Pause' : 'Resume'} ${rule.name}`}
                  disabled={saving}
      ...
      {ordered?.length === 0 && !rules.error && (
        ...
        >
          ...
          >
            <CalendarDays size={32} color={tokens.primary} />
          </View>
      ...
      {adding && (
        ...
        >
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            ...
            <Button
              style={{ flex: 1 }}
              disabled={saving || !selected || !name.trim() || !amount.trim()}

...
import {
  permissionStatus,
  reconcileRecurringNotifications,
...
export default function NotificationSettingsScreen() {
  ...
  const [permission, setPermission] = React.useState<'granted' | 'denied' | 'undetermined' | null>(
    null,
  );
  ...
  React.useEffect(() => {
    if (settings.data?.[0])
      setPreferences(normalizeNotificationPreferences(settings.data[0].notificationPreferences));
  }, [settings.data]);
  React.useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {!settings.loading && !settings.error && !settings.data?.[0] && (
        <Text style={{ color: tokens.foregroundMuted }}>
          Connect once to load your account settings.
        </Text>
      )}
      {preferences && (
        <View>
      ...
      <View
        style={{ gap: 10, paddingTop: 12, borderTopWidth: 1, borderColor: tokens.borderSubtle }}
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        ...
        {permission === 'undetermined' && (
          <Button variant="outline" onPress={() => void enableDeviceDelivery()}>

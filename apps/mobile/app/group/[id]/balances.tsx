...
export default function BalancesScreen() {
  ...
  const entries = ledger
    ? [...ledger.names].map(([memberId, name]) => ({
        id: memberId,
        name: memberId === userId ? 'You' : name,
  ...
  return (
    ...
    >
      ...
      {!id ? (
        <Empty title="Missing group ID" description="Open balances from a group." />
      ) : !group && !loading ? (
        ...
        <View style={{ gap: 12 }} accessibilityRole="alert">
          <Empty
            ...
            description="The complete group ledger could not be loaded. No partial balance is shown."
          />
          <Button onPress={retry}>Retry</Button>
        </View>
      ) : loading || !ledger ? (
        <Typography variant="heading">Loading all-time balances…</Typography>
      ) : (
        <>
            ...
            <View style={{ gap: 16 }}>
              ...
              {entries.map((member) => (
                ...
                >
                  ...
                  <Typography variant="small" style={{ fontVariant: ['tabular-nums'] }}>
                    {formatMinor(member.amountMinor, ledger.currency)}
                  </Typography>
                </View>
              ))}
            </View>

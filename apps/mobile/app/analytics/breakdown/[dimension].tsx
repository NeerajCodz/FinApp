...
export default function AnalyticsBreakdownScreen() {
        ...
        const resolved =
          dimension === 'category'
            ? categories.get(transaction.categoryId ?? '')
              ? recordId(categories.get(transaction.categoryId ?? '')!)
              : UNCATEGORIZED_ID
            : dimension === 'account'
              ? accounts.get(transaction.accountId ?? '')
                ? recordId(accounts.get(transaction.accountId ?? '')!)
                : UNASSIGNED_ACCOUNT_ID
  ...
  return (
    ...
    >
      ...
      (result && !result.item && rangeState.covered) ? (
        recover
      ) : (
        <>
          {error && (
          ...
          {rangeState.refreshing && (
            <Typography variant="caption">Refreshing transactions…</Typography>
          )}
          {(!result || (!result.item && !rangeState.covered)) && !error ? (
            ...
            result.item && (
              <>
                ...
                <Typography variant="caption">
                  {result.totalMinor > 0n
                    ? Number((result.item.amountMinor * 1000n) / result.totalMinor) / 10
                    : 0}
                  % of spending · {period}
                </Typography>

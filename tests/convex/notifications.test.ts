...
describe('notification delivery', () => {
  it('filters at the producer, deduplicates event keys, and persists read state', async () => {
    ...
    const noEvents = await t.run((ctx) =>
      ctx.db
        .query('notifications')
    ...
    await authenticated.mutation(api.notifications.mutations.setPreferences, {
      preferences: { ...defaultNotificationPreferences, budget: true },
      clientMutationId: 'prefs-1',
    });
    await t.run((ctx) =>
      createNotification(
        ctx,
    ...
    await authenticated.mutation(api.notifications.mutations.markRead, {
      notificationId: events[0]!._id,
      clientMutationId: 'read-1',
    });
    ...
    const afterReplay = await t.run((ctx) =>
      ctx.db
        .query('notifications')
        .withIndex('by_recipient_createdAt', (query) => query.eq('recipientId', userId))
        .collect(),

...
export type AppLock =
  | { method: 'device' }
  | { method: 'passcode'; salt: string; digest: string };
...
export async function readPasscodeThrottle(userId: string): Promise<{ attempts: number; retryAt: number }> {
  ...
  if (!value || typeof value !== 'object' || !('attempts' in value) ||
      !('retryAt' in value) || typeof value.attempts !== 'number' ||
      typeof value.retryAt !== 'number' || !Number.isInteger(value.attempts) ||
      value.attempts < 0 || value.attempts > 4 || !Number.isFinite(value.retryAt))
    throw new Error('The app passcode attempt limit could not be read.');
  return { attempts: value.attempts, retryAt: value.retryAt };
}

export async function recordFailedPasscode(userId: string): Promise<{ attempts: number; retryAt: number }> {
  const previous = await readPasscodeThrottle(userId);
  if (previous.retryAt > Date.now()) return previous;
  const attempts = previous.attempts + 1;
  const next = attempts >= 5
    ? { attempts: 0, retryAt: Date.now() + 30_000 }
    : { attempts, retryAt: 0 };
...
export async function createPasscodeLock(code: string): Promise<AppLock> {
  ...
  const salt = Array.from(await Crypto.getRandomBytesAsync(32), (byte) => byte.toString(16).padStart(2, '0')).join('');
...
export async function authenticateDevice(): Promise<boolean> {
  ...
  if (result.error === 'user_cancel' || result.error === 'system_cancel' || result.error === 'app_cancel') return false;

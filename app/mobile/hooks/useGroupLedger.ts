...
export function useGroupLedger(id: string | undefined) {
  ...
  const projected = useMemo(() => {
    if (!group || !members.data || !payers.data || !participants.data ||
      !range.covered || !range.transactions || !range.settlements) return null;
    try {
      return { value: projectGroupBalances(group, members.data, range.transactions, payers.data, participants.data, range.settlements) };
    } catch (error) {
      return { error: error instanceof Error ? error : new Error('GROUP_LEDGER_UNAVAILABLE') };
    }
  }, [group, members.data, payers.data, participants.data, range.covered, range.transactions, range.settlements]);
  const error = groups.error || members.error || payers.error || participants.error || range.error || projected?.error;
  return {
    group, members: members.data, ledger: projected?.value, error,
    loading: !!userId && (!groups.data || !members.data || !payers.data || !participants.data ||
      !range.covered || !range.transactions || !range.settlements) && !error,
    refreshing: range.refreshing,
    retry: () => { groups.retry(); members.retry(); payers.retry(); participants.retry(); range.retry(); },
    userId,
  };
}

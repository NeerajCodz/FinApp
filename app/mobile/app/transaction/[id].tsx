...
export default function TransactionDetailScreen() {
  ...
  const destination = accounts.get(typeof record?.transferAccountId === 'string' ? record.transferAccountId : '');
  const timeZone = typeof profileState.data?.[0]?.timezone === 'string' ? profileState.data[0].timezone as string : 'UTC';
  const error = transactionState.error || accountState.error || categoryState.error || profileState.error;
  const loading = !transactionState.data || !accountState.data || !categoryState.data || !profileState.data;
  const duplicable = transaction && ['expense', 'income', 'transfer'].includes(transaction.type) &&
    transaction.amountMinor > 0n && account && account.archivedAt === undefined &&
    account.currency === transaction.currency && (transaction.type === 'transfer'
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: tokens.background }}
    ...
    {!id ? <Empty title="Missing transaction ID" description="Open a transaction from Activity to view its details." />
      : error && loading ? <View style={{ gap: 12 }} accessibilityRole="alert">
      ...
      : loading ? <Typography variant="heading">Loading transaction…</Typography>
      : !transaction || record?.deletedAt !== undefined ? <Empty title="Transaction unavailable" description="This transaction was removed or is no longer saved on this device." />
      : <>
        {error && <View style={{ gap: 8 }} accessibilityRole="alert">
        ...
        <View>
          ...
          <Separator />
          <SettingsRow label="Note" value={typeof record?.note === 'string' && record.note.trim() ? record.note : 'None'} />
        </View>
        {duplicable && <Button size="lg" variant="outline" onPress={duplicate} style={{ justifyContent: 'space-between' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            <ReceiptText size={19} color={tokens.foreground} />
            <Text style={{ color: tokens.foreground, fontFamily: 'SpaceGrotesk_600SemiBold', fontSize: 15 }}>Duplicate transaction</Text>
          </View>

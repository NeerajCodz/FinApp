...
import { Button, Empty, IconButton, Input, Tabs, Typography } from '@/components/ui';
import { useTheme } from '@/providers/ThemeProvider';
import { layoutTokens } from '@/lib/theme/tokens';
import { aggregateAnalytics, getAnalyticsRange, type AnalyticsPeriod } from '@convex/analytics/domain';
...
export default function ActivityScreen() {
  ...
  const rangeState = useLocalTransactionRange<LocalRecord>(userId, range.startAt, range.endAt, fetchTransactionRange);
  ...
  const visible = useMemo(() => {
    ...
    const rows = rangeState.data.flatMap((record) => {
      ...
      if (!transaction || !id || transaction.deletedAt !== undefined ||
        transaction.occurredAt < range.startAt || transaction.occurredAt >= range.endAt) return [];
      const kind: ActivityKind = record.groupId && transaction.type === 'expense' ? 'group' : transaction.type;
      return [{ id, ownerId: userId ?? '', kind, occurredAt: transaction.occurredAt,
        merchant: transaction.merchant, amountMinor: transaction.amountMinor }];
    });
    const needle = query.trim().toLocaleLowerCase();
    return filterActivity(rows, filter).filter((row) => {
  ...
  const totals = useMemo(() => aggregateAnalytics(
    visible.flatMap((record) => { const transaction = ledgerTransaction(record); return transaction ? [transaction] : []; }),
    analyticsEntities(categoryState.data ?? []), currency, period, range.startAt, range.endAt,
    timeZone, analyticsEntities(accountState.data ?? []),
  ...
  const ready = !!profile && !!accountState.data && !!categoryState.data && !!rangeState.data &&
    (rangeState.covered || rangeState.data.length > 0);
  return <ScrollView style={{ flex: 1, backgroundColor: tokens.background }}
    contentContainerStyle={{ paddingHorizontal: 20, paddingTop: insets.top + 16, paddingBottom: layoutTokens.sectionGap, gap: 24 }}
    keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    {searching ? <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
      <Input accessibilityLabel="Search transactions" autoFocus placeholder="Title, merchant, category, amount…"
        value={query} onChangeText={setQuery} style={{ flex: 1 }} />
      <IconButton label="Close search" variant="ghost" onPress={() => { setQuery(''); setSearching(false); }}>
    ...
    {ready && <View style={{ gap: 12 }}>
      <Typography variant="label">{period} · {currency}</Typography>
      <MetricPair left={{ label: 'Spent', value: formatMinor(totals.spentMinor, currency) }}
        right={{ label: 'Income', value: formatMinor(totals.incomeMinor, currency) }} />
      <Typography variant="caption">Totals include posted transactions in {currency} only.</Typography>
    </View>}
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ minWidth: '100%' }}>
      <Tabs value={filter} onChange={(value) => setFilter(value as ActivityFilter)}
        tabs={filters.map((value) => ({ label: value, value }))} />
    </ScrollView>
    {error && <View style={{ gap: 10 }} accessibilityRole="alert">
      <Typography variant="small">{ready ? 'Showing saved activity. Refresh failed; totals may be incomplete.' : 'Activity is unavailable.'}</Typography>
      <Button onPress={() => { profileState.retry(); accountState.retry(); categoryState.retry(); rangeState.retry(); }}>Retry</Button>
    </View>}
    {rangeState.refreshing && <Typography variant="caption">Refreshing activity…</Typography>}
    {!userId || (profileState.data && !profile) ? <Empty title="Activity unavailable" description="Sign in to see your ledger." />
      : !ready && !error ? <Typography variant="heading" accessibilityLabel="Loading activity">Loading activity…</Typography>
      : ready && sections.length === 0 && rangeState.covered ? <Empty title={query ? 'No search matches' : 'No activity this period'}
        description={query ? 'Try a merchant, title, category, account, or amount.' : 'No transactions match this period and filter.'} />
      : ready && sections.length === 0 ? <Typography variant="small">No matching saved rows. Refresh to confirm the full period.</Typography>
      : ready && sections.map(([date, records]) => <DateSection key={date} title={date}>
        {records.map((record) => {
